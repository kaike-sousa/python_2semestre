#APRENDIZADO
""" 
Marcar a primeira linha depois do def com a bolinha vermelha nas linhas(breackpoint) 
ir no menu de rodar, escolher o debug
usar o watch do debbuger para avaliar expressoes
"""
...
#opção simplificada
def media(lista):
    soma_total = sum(lista)                #Python tem a propriedade sum que soma os itens da lista
    quantidade = len(lista)            #Python tem a propriedade len que é quantidade de itens da lista
...
#     cartas_sem_naipe = ['A', '2', '3',...]
#     cartas_com_naipe = []
#     for carta_sem_naipe in cartas_sem_naipe:
#         carta_pronta = carta_sem_naipe + naipe
#         cartas_com_naipe.append(carta_pronta)
#     return cartas_com_naipe

#jeito prático
def cria_naipe(naipe):
    cartas_sem_naipe = ['A', 'J', 'Q', 'K']
    cartas_com_naipe = []
    for carta_sem_naipe in cartas_sem_naipe:
        carta_pronta = carta_sem_naipe + naipe
        cartas_com_naipe.append(carta_pronta)
    for carta_sem_naipe in range(2,11):
        carta_pronta = str(carta_sem_naipe) + naipe
        cartas_com_naipe.append(carta_pronta)
    return cartas_com_naipe
'''
